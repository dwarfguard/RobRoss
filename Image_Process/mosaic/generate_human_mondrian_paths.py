"""human_mondrian route: same Mondrian-style painting as image_to_mondrian,
but the command sequence is augmented with human-like "performance flourishes"
— repeated strokes, swooping travel arcs, test dabs, edge touch-ups, idle
shakes — so the robot looks like a real painter, not a machine.
    ...
    Config profile (configs/*.json)
      ...
      -> performance_flourish.apply_flourishes() (inject human-like micro-movements)
      -> generate_human_mondrian_paths.py       -> output/<painting_paths_file> (+ preview SVG + quantized preview PNG)

Output uses the exact same command vocabulary as the other routes
...
from performance_flourish import apply_flourishes

DEFAULT_CONFIG_FILE = "configs/human_mondrian_demo_a4.json"
...
def build_painting_paths(
    ...
    return {
        ...
        "style": config.get("style", "human_mondrian"),
        "version": "0.1",
        "config": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert a source photo into a 5-color Mondrian-style painting_paths.json with human-like performance flourishes."
    )
    parser.add_argument(
        "--config",
    ...
    # --- Inject human-like performance flourishes ---------------------------
    flourish_cfg = config.get("performance_flourish")
    if flourish_cfg:
        commands = apply_flourishes(commands, canvas, flourish_cfg)
