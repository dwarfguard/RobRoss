...
# Resolve the default config relative to the repo root (two levels up from
# this script: Image_Process/glass/ -> RobRoss/), so the program works
# regardless of which directory it is invoked from.
_SCRIPT_DIR = Path(__file__).resolve().parent
_REPO_ROOT = _SCRIPT_DIR.parent.parent  # Image_Process/glass -> RobRoss
DEFAULT_CONFIG_FILE = str(_REPO_ROOT / "configs" / "glass_a4.json")
...
# --- Config-driven color mapping (glass_polygon / custom styles) ---------


def _get_standard_colors(config: dict) -> dict:
    """Return the standard-colors dict used for high-contrast recognition.

    When the config carries a ``color_mapping.standard_colors`` block it is
    used directly.  Otherwise the hardcoded 7-colour set (backwards-compatible
    with the original colourful / image_to_mondrian configs) is returned.
    """
    color_mapping = config.get("color_mapping")
    if color_mapping and color_mapping.get("standard_colors"):
        return dict(color_mapping["standard_colors"])

    # Hardcoded fallback – same set that was originally in main().
    return {
        "white": "#ffffff",
        "blue": "#0000ff",
        "black": "#000000",
...
def apply_standard_colors_for_recognition(config: dict) -> list:
    """Replace palette hex values with high-contrast standard colours *in-place*
    and return the original physical palette so it can be restored later.

    Returns the deep-copied physical palette (list of {name, hex} dicts).
    """
    import copy

    physical_colors = copy.deepcopy(config["palette"]["colors"])
    standard_colors = _get_standard_colors(config)
...
def resolve_color_mapping(
    regions: list,
    config: dict,
    physical_hex_dict: dict,
    label_image: "np.ndarray",
    name_to_index: dict,
) -> tuple:
    """Map recognised-region colours to physical paint colours.

    Returns ``(all_combined_regions, label_image_combined)``.

    **Config-driven path** (``color_mapping`` key present in config):
      - ``region_mapping``  – standard-name → physical-name for the base fill
      - ``overlay_mapping`` – standard-name → physical-name for a second pass
      - ``drop_colors``     – list of standard names whose fill is skipped

    **Hardcoded fallback** (no ``color_mapping`` key):
      Identical to the previous main() logic (purple→red+blue,
      orange→red+yellow, green→blue+yellow, brown→red+black, gray dropped).
    """
    import copy
    ...
    if color_mapping is not None:
        region_mapping = color_mapping.get("region_mapping", {})
        overlay_mapping = color_mapping.get("overlay_mapping", {})
        drop_colors = set(color_mapping.get("drop_colors", []))

        for r in regions:
            name = r["color_name"]

            if name in drop_colors:
                continue
            ...
            if name in region_mapping:
                target_name = region_mapping[name]
            elif name in physical_hex_dict:
                target_name = name
            else:
                continue  # unmapped colour → skip

            r1["color_name"] = target_name
            r1["color_hex"] = physical_hex_dict.get(target_name, r["color_hex"])
            all_combined_regions.append(r1)

            # --- overlay layer (optional) ---
            if overlay_mapping and name in overlay_mapping:
                overlay_name = overlay_mapping[name]
                r2 = copy.deepcopy(r)
                r2["color_name"] = overlay_name
                r2["color_hex"] = physical_hex_dict.get(overlay_name, r["color_hex"])
                all_combined_regions.append(r2)

            # --- quantised preview ---
            if target_name in name_to_index:
                label_image_combined[r["mask"]] = name_to_index[target_name]

        return all_combined_regions, label_image_combined

    # ── hardcoded fallback (original colourful mapping) ───────────────
...
# --- Stained-glass region subdivision ----------------------------------


def subdivide_regions(regions: list, cell_size_px: int, image_shape: tuple, skip_colors: set = None) -> list:
    """Split large regions into irregular Voronoi polygons for a natural
    stained-glass look – each piece is a different size and shape, never
    a perfect square.

    Random seed points are placed inside each region and every pixel is
    assigned to its nearest seed, producing organic, hand-cut-looking
    polygon cells.  The cells are clipped to the region's own boundary so
    the overall shape is preserved.

    Regions whose colour name is in *skip_colors* are kept whole (the
    background stays as one uninterrupted pane).
    """
    import numpy as np
    ...
    min_cell_area = max(10, cell_size_px * cell_size_px // 2)
    rng = np.random.RandomState(42)  # reproducible subdivision
    result = []

    for region in regions:
        mask = region["mask"]
        area_px = int(region.get("area_px", np.sum(mask)))

        if area_px <= min_cell_area or region.get("color_name") in skip_colors:
            result.append(region)
            continue

        # --- Voronoi subdivision within this region ---
        ys, xs = np.where(mask)
        n_pixels = len(ys)
        pixel_pts = np.column_stack([xs, ys]).astype(np.float32)

        # Number of cells proportional to region area
        n_cells = max(2, int(round(n_pixels / max(cell_size_px * cell_size_px, 1))))
        n_cells = min(n_cells, max(2, n_pixels // 20))  # never too fine

        # Random seed points drawn from the region's own pixels
        seed_idx = rng.choice(n_pixels, n_cells, replace=False)
        seeds = pixel_pts[seed_idx]  # (n_cells, 2)

        # Assign each pixel to its nearest seed (batched to stay in memory)
        labels = np.empty(n_pixels, dtype=np.int32)
        BATCH = 20000
        for b in range(0, n_pixels, BATCH):
            batch = pixel_pts[b:b + BATCH]
            diff = batch[:, None, :] - seeds[None, :, :]       # (B, cells, 2)
            dist = np.sum(diff.astype(np.int64) ** 2, axis=2)   # squared Euclidean
            labels[b:b + len(batch)] = np.argmin(dist, axis=1)

        # Build a sub-region for each Voronoi cell
        for cell_id in range(n_cells):
            keep = labels == cell_id
            cell_count = int(np.sum(keep))
            if cell_count < 4:
                continue

            sub_mask = np.zeros_like(mask)
            sub_mask[ys[keep], xs[keep]] = True

            sub = dict(region)
            sub["mask"] = sub_mask
            sub["area_px"] = cell_count
            result.append(sub)
...
def main() -> None:
    ...
    # Resolve paths in the config that are relative to the repo root, so the
    # program works when invoked from any directory.
    output = config["output"]
    output_dir = _REPO_ROOT / output["directory"]
    ...
    image_path = _REPO_ROOT / config["source_image"]["path"]
    # Update the config so build_regions() (which reads source_image["path"]
    # directly) uses the resolved absolute path.
    config["source_image"]["path"] = str(image_path)

    # ── Color recognition: swap in high-contrast standard hex values ──
    physical_colors = apply_standard_colors_for_recognition(config)

    # Quantize + segment with standard colours, then restore physical palette.
    regions, dropped_count, label_image, image_size, scale_mm_per_px, offset_mm = build_regions(config)

    # ── Stained-glass subdivision (optional, config-driven) ─────────
    subdivide_mm2 = config["segmentation"].get("subdivide_cell_size_mm2")
    if subdivide_mm2:
        cell_size_px = max(1, int(round(subdivide_mm2 ** 0.5 / scale_mm_per_px)))
        # Keep background colours whole – only subdivide subject matter.
        skip_subdivide = set(config["segmentation"].get("subdivide_skip_colors", ["white"]))
        regions_before = len(regions)
        regions = subdivide_regions(regions, cell_size_px, label_image.shape, skip_subdivide)
        dropped_count += max(0, regions_before - len(regions))
    ...
    # Map recognised-region colours to physical paint colours
    # (config-driven when "color_mapping" is present, hardcoded fallback otherwise).
    all_combined_regions, label_image_combined = resolve_color_mapping(
        regions, config, physical_hex_dict, label_image, name_to_index
    )

    # ── Fill strokes + border strokes + ordering ──────────────────────
