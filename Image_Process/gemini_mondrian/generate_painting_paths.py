...
(select_tool/dip_paint/move_to/lower_tool/paint_stroke/paint_path/lift_tool),
so it is consumable by the same ros2/robross_painter executor. Scanline
fills emit one paint_stroke per row; traced borders emit one continuous
paint_path per contour (like the line_art route) so the robot draws each
border as a single trajectory instead of one settle cycle per segment.
"""
