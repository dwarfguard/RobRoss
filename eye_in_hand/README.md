...
> **采集参数要求（重要）**
> - 相机距标记 **10~25cm**：D405 建议工作距离 7~30cm，太近（<7cm）
>   失焦会让 solvePnP 的深度/位置误差成倍放大；
> - 俯视角**适中**（相机光轴与标记法线夹角 20°~60°）：太正看不出
>   旋转多样性，太斜（>70°）时平面标记的深度解算会退化；
> - 每组摆好后停 1 秒、机械臂完全静止再按 `[Space]`；
> - 采完一组如果觉得不好，按 **`[Backspace]` 删除最后一组**再重采。

### 标记尺寸（常见坑）

solvePnP 用的是**黑色方块**的边长，程序默认 `--marker-size 0.02`（20mm）。
如果标定标记打印得更大（例如整页打印、黑边 40~50mm），必须传
`--marker-size <实测黑边米>`，否则所有距离都会被算错——典型症状就是
「旋转误差很小（<1°）但位置误差十几毫米」。

程序现在会自动反推实际黑边尺寸。实时质量报告或最终结果里如果出现：

```
⚠ 反推标记黑边 ≈ 47 mm (当前参数 20 mm)
```
...
注意两个阶段的标记是**不同的**：
- `calibrate_eih.py` 的 `--marker-size` = **标定用大标记**的黑边
  （例如 48mm → `--marker-size 0.048`）；
- `aruco_eih.py` 画布检测用纸上 4 角的**小标记**（20mm），
  保持默认 `0.02` 即可（`start_painting_eih.sh` 默认也是 0.02）。

### 摄像头打开方式（重要）

程序**优先用 pyrealsense2 SDK** 打开 D405：

```
[✓] 使用 pyrealsense2 SDK 打开 D405 (1280×720, 实时内参)
```

SDK 路径会强制 1280×720（D405 原生模式）并直接使用该流的实时内参
（含出厂畸变），与标定文件完全一致，精度最高。

如果显示下面这行，说明 SDK 不可用、回退到了 OpenCV UVC：

```
[i] pyrealsense2 不可用或未连接 D405，回退 OpenCV UVC
```

UVC 模式下 D405 常以 640×480（传感器裁剪模式）输出，而内参是
1280×720 的，程序只能**近似**缩放内参，标定和检测的位置误差会明显
变大——请安装 pyrealsense2 并优先走 SDK 路径：
...
如果旋转轴分布已合理（x/y/z 都不接近 1.00），但一致性误差仍然大
（位置 > 15mm 或旋转 > 5°），按顺序检查：

1. **摄像头必须走 pyrealsense2 SDK 路径**（打印 `使用 pyrealsense2 SDK
   打开 D405 (1280×720)`）。D405 的 UVC 640×480 是传感器裁剪模式，
   和 1280×720 出厂内参不匹配，缩放内参只能近似；SDK 路径强制
   1280×720 并直接使用该流实时内参，一劳永逸解决分辨率问题。
2. **按 Space 时机械臂必须完全静止**。程序会在按下瞬间重新读取 TCP，
   但拖动中的微小位移仍会污染数据；每次摆好位姿后停 1 秒再按。
3. **相机固定要牢**。D405 装在末端若会晃动，每个位姿的 `T_ee_cam` 都不同，
   误差不会随组数增加而收敛。
4. **标记不能移动**。标定过程中纸或标记板被碰动，会直接破坏一致性。
5. **畸变系数不要用全零**。旧版 `read_realsense_intrinsics.py` 把出厂
   畸变系数写成了零；请重新运行它生成 `camera_calib.json`（现在会写入
   出厂畸变系数），或用棋盘格重新标定。近距离宽角镜头下，畸变不校正
   会造成几毫米到几十毫米的位置误差。
