...
当前仅使用桌面上方的一个固定 D405 摄像头（eye-to-hand 布局），不在机械臂末端加装第二个摄像头。

### 当前方案已满足 Demo v1

Demo v1 的目标是证明机器人能用笔在 A4 纸上画线稿，是开环执行。固定摄像头完成三件事：

1. 检测 4 个 ArUco 标记 → 定位纸张位置和姿态
2. 手眼标定 → 得到 `T_base_cam`（相机→基座坐标变换）
3. 生成 `canvas_calibration.yaml` → ROS 2 画画使用

所有路径离线生成，画画过程中不需要视觉反馈，固定摄像头已足够。

### 不装臂端摄像头的理由

| 原因 | 说明 |
|------|------|
| 额外硬件成本 | 需要超轻摄像头（<30g）、专用支架、线缆走线 |
| 标定复杂度 | eye-in-hand 需要单独的标定流程，增加一套变换矩阵 |
| 末端负载限制 | D405 太重，臂端只能装更轻的型号，精度和功能会打折扣 |
| 软件复杂度 | 需融合两个摄像头的坐标系统，调试工作量大幅增加 |

### 未来臂端摄像头的潜在价值

如果下一阶段想提升自动化程度，臂端摄像头最有价值的三个功能：
...
> **结论：Demo v1 不需要加装第二个摄像头。但如果下一阶段要做自动 TCP 标定或防碰纸偏移，臂端摄像头是最有性价比的下一步。**
